...
class Memberlog extends Controller
{
    ...
    public function index()
    {
        ...
        if(isset($key['act']) && $key['act'] == 'check')
        {
            ...
            {
                $where[] = ['l.create_time','>=',strtotime($key['start_time'].' 00:00:00')];
            }
            else if(!$key['start_time'] && $key['end_time'])
            {
                $where[] = ['l.create_time','<=',strtotime($key['end_time'].' 23:59:59')];
            }
            else if($key['start_time'] && $key['end_time'])
            {
                ...
                {
                    $where[] = ['l.create_time','>=',strtotime($key['end_time'].' 00:00:00')];
                    $where[] = ['l.create_time','<=',strtotime($key['start_time'].' 23:59:59')];
                }
            }
