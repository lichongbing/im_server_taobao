...
class System extends Controller
{
    ...
    protected function getBasicConfig(){
        ...
        $field_val = json_decode($info['field_val'],true);
        return [
            Form::input('user_default_friend','注册默认成为好友的会员ID',isset($field_val['user_default_friend']) ? $field_val['user_default_friend']:'')->type('text')->col(13)->info("注册默认成为好友的会员ID"),
            ...
            Form::radio('user_regiter_code_status', '邀请码是否必须', $field_val['user_regiter_code_status'] ?? 0)->options([['label' => '是', 'value' => 1],['label' => '否', 'value' => 0]])->col(13),
            Form::radio('user_regiter_code_type', '邀请码类型', $field_val['user_regiter_code_type'] ?? 1)->options([['label' => '大小写字母和数字', 'value' => 1],['label' => '大小写字母', 'value' => 2],['label' => '纯数字', 'value' => 3]])->col(13),
            Form::input('user_regiter_code_num','邀请码位数',isset($field_val['user_regiter_code_num']) ? $field_val['user_regiter_code_num']:'')->type('number')->col(13)->info("请输入邀请码位数"),
