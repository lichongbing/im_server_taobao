...
class In
{
    ...
    public function reg()
    {
        ...
        //邀请码是否必填
        $code_status = BsysConfig::getVal('basic_config', 'user_regiter_code_status');
        if($code_status == 1){
            if(empty($post_data['code'])){
                $return_data['msg'] = '邀请码必填';
                return json($return_data);
            }
        }
        ...
        //邀请码是否正确
        if(!empty($post_data['code'])){
            $parent_user = User::where('code', $post_data['code'])->find();
            if(empty($parent_user)){
                $return_data['msg'] = '邀请码不正确';
                return json($return_data);
            }else{
                $insert['parent_id'] = $parent_user['id'];
            }
        }
        
        //邀请码生成
        $code_type = BsysConfig::getVal('basic_config', 'user_regiter_code_type');
        $code_num = BsysConfig::getVal('basic_config', 'user_regiter_code_num');
        for($i=0;$i<10;$i++){
            $code = $this->randStr($code_num,$code_type);
            $code_user = User::where('code', $code)->find();
            if(empty($code_user)){
                $insert['code'] = $code;
                break;
            }
...
//                            'last_chat_time'=>time(),
                        ...
                        $text = $text ? $text : '欢迎来到IM工作室,有什么问题您都可以联系我!!';
...
//                        ChatList::where(['list_id'=>$list_id,'user_id'=>$user->id])->update(['last_chat_time'=>time()]);
    ...
    //随机产生六位数邀请码Begin
    public function randStr($len=6,$format='1') {
         switch($format) { 
         case '1':
