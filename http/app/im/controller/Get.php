...
class Get
{
    ...
    public function base()
    {
        ...
        $kefu_id = (int)BsysConfig::getVal('basic_config', 'user_default_friend');
        $chat_list = ChatList::where(['user_id'=>USER_ID,'user_ids'=>json_encode([$kefu_id,USER_ID])])->find();
        $return_data = [
            'err' => 0,
            'msg' => 'success',
            'data' => [
...
//                    ->toArray();
    ...
    public function inviteList(){
        $post_data = Request::post();
        $users = User::where(['parent_id'=>$post_data['user_id']])->select()->toArray();
        foreach ($users as &$value){
            $user_state_obj = UserState::field('photo')->where('user_id', $value['id'])->find();
            $face = getShowPhoto($user_state_obj, $value['sex'], $value['id'], 300);
            $value['face'] = $face;
        }
        $return_data['err'] = 0;
        $return_data['msg'] = '成功';
