...
//use extend\alysms\Client;
use extend\aliysms\api\sendSms;
...
//        if (RedisService::get($key)) return JsonDataService::fail('60秒后重新发送!');
        ...
        $sendSms = new sendSms;
        $res = $sendSms->sendSmss($config['sms_appkey'],$config['sms_appScript'],$mobile,$config['sms_sign'],$config['sms_code'],$code);
        $res = (array)$res;
        if ($res['Message'] != 'OK') {
